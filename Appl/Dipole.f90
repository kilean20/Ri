...
!        print*,"dipole: ",pos,extfld(5)
        ...
        subroutine Bpol_Dipole(h0,h1,tanphi,tanphib,k1,psi,ptarry1,ang,Nplocal)
        implicit none
        include 'mpif.h'
        integer, intent(in) :: Nplocal
        double precision, pointer, dimension(:,:) :: ptarry1
...
!        subroutine Bpol_Dipole(h0,h1,tanphi,tanphib,k1,ptarry1,ptarry2)
!        implicit none
!        include 'mpif.h'
!        double precision, dimension(6), intent(in) :: ptarry1
!        double precision :: h0,h1,tanphi,tanphib,k1
!        double precision, dimension(6), intent(out) :: ptarry2
!        double precision :: tanphi2,secphi2,secphi,secphi3,tanphi3
!
!        tanphi2 = tanphi**2
!        tanphi3 = tanphi**2*tanphi
!        secphi2 = 1.0 + tanphi2
!        secphi = sqrt(secphi2)
!        secphi3 = secphi2*secphi
!
...
!                     k1*tanphi-0.5d0*h0**2*tanphi3)*ptarry1(1)**2-&
!                     (0.5d0*h0*h1*secphi3+&
!                     k1*tanphi + 0.5d0*h0**2*tanphi3)*&
...
!                     ptarry1(3)-(h0*h1*secphi3+2*k1*tanphi-h0**2*tanphi*&
!                     secphi2)*ptarry1(1)*ptarry1(3)
!        ptarry2(5) = ptarry1(5)
!        ptarry2(6) = ptarry1(6)
!
!        end subroutine Bpol_Dipole

        subroutine Sectorold_Dipole(len,beta,h0,k1,ptarry1,Nplocal,qm0)
        implicit none
        include 'mpif.h'
        integer, intent(in) :: Nplocal
        double precision, pointer, dimension(:,:) :: ptarry1
...
!                     ptarry1(5,i) + h0**2*j1*ptarry1(6,i)
        ...
        end subroutine Sectorold_Dipole
        ...
        double precision :: kx2,kx,cx,sx,dx,j1,ky2,ky,cy,sy,dy,&
                            gambet,gam2,qmrel
        real*8 :: tmppx,tmppy,tmppt,tmph,gamma0,gambetz,gamn,deltaz
        integer :: i
...
! see p.147 of F. Iselin paper
          qmrel = (ptarry1(7,i)-qm0)/qm0
          tmppt = -(ptarry1(6,i)+qmrel)*beta*gambet
          gamn = gamma0 - tmppt
          gambetz = sqrt((gamn**2-1)/(1+ptarry1(2,i)**2+ptarry1(4,i)**2))
          tmppx = ptarry1(2,i)*gambetz
          tmppy = ptarry1(4,i)*gambetz
          tmph = sqrt((tmppt-gamma0)**2-1.0d0-tmppx**2-tmppy**2)
          deltaz = (1./beta+(tmppt-gamma0)/tmph)*len*beta

          !ptarry2(5) = -h0*sx*ptarry1(1,i)-h0*dx*ptarry1(2,i)+&
          !           ptarry1(5,i) - h0**2*j1*ptarry1(6,i) + &
          !           len/gam2*(ptarry1(6,i)+qmrel)
          !using nonlinear exact drift instead of linear drift
          ptarry2(5) = -h0*sx*ptarry1(1,i)-h0*dx*ptarry1(2,i)+&
                     ptarry1(5,i) - h0**2*j1*ptarry1(6,i) + deltaz
                     !ptarry1(5,i) - h0**2*j1*ptarry1(6,i) + deltaz 
! Transport defines (5) as path differnce which is v dt
!          ptarry2(5) = h0*sx*ptarry1(1,i)+h0*dx*ptarry1(2,i)+&
!                     ptarry1(5,i) + h0**2*j1*ptarry1(6,i)
