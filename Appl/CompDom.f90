...
!        do i = 0, nprocrow - 1
!          do j = 0, nproccol - 1
!              this%LcTabnm(1,i,j) = nz/nprocrow
!              this%LcTabnm(2,i,j) = ny/nproccol
!          enddo
!        enddo
        if(Flagbc.eq.2) then !(even,even,odd)
          if(nprocrow.gt.1) then
          do j = 0, nproccol - 1
              this%LcTabnm(1,nprocrow-1,j) = nz/nprocrow+1
          enddo
          endif
        !(odd,odd,even)
          ...
          if(nproccol.gt.1) then
          do i = 0, nprocrow - 1
              this%LcTabnm(2,i,nproccol-1) = ny/nproccol+1
          enddo
          endif
        !(odd,odd,odd)
...
!        call MPI_BARRIER(commrow,ierr)
        ...
        if((Flagbc.eq.1).or.(Flagbc.eq.8)) then
          ...
          zmin = temp1(3)-epsz*(temp2(3)-temp1(3))
          zmax = temp2(3)+epsz*(temp2(3)-temp1(3))
        else if((Flagbc.eq.4).or.(Flagbc.eq.6).or.(Flagbc.eq.7)) then
          xmin = temp1(1)
          xmax = temp2(1)
          ymin = temp1(2)
          ymax = temp2(2)
        ...
        !update geometry parameters using new particle positions.
        !only global range and mesh size are updated
        subroutine updateglb_CompDom(this, ptrange, grid2d, Flagbc)
        implicit none
        include 'mpif.h'
        type (CompDom), intent(inout) :: this
        type (Pgrid2d), intent(in) :: grid2d
...
!        endif
        ...
        end subroutine updateglb_CompDom
