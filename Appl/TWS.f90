...
!----------------------------------------------------------------
      ...
      contains
        ...
        !write(16,*)t,ein,ein2,ein*costhi+ein2*costhi2
...
!              (abs(zlc-zend1).le.epstol) ) then
            ...
            dd = Fcoef(9+numpar1) !shift of first standing wave
            ez12 = Fcoef(ntmp+1)/2
            ezp12 = 0.0
            ezpp12 = 0.0
            do i = 2,(numpar1-1)/2+1
              ez12 = ez12 + Fcoef(2*i-2+ntmp)*&
                cos((i-1)*2*pi*(zz+dd)/zlen) + &
                Fcoef(2*i-1+ntmp)*sin((i-1)*2*pi*(zz+dd)/zlen)
              ezp12 = ezp12 + (i-1)*2*pi/zlen*&
                (-Fcoef(2*i-2+ntmp)*sin((i-1)*2*pi*(zz+dd)/zlen)+&
                Fcoef(2*i-1+ntmp)*cos((i-1)*2*pi*(zz+dd)/zlen))
              ezpp12 = ezpp12+((i-1)*2*pi/zlen)**2*&
                (-Fcoef(2*i-2+ntmp)*cos((i-1)*2*pi*(zz+dd)/zlen)&
                - Fcoef(2*i-1+ntmp)*sin((i-1)*2*pi*(zz+dd)/zlen))
            enddo
            ez12=ez12*escale
            ezp12=ezp12*escale
...
!        call flush_(14)
        ...
        !get the E field on the axis
        subroutine getaxfldEold_TWS(z,this,ez1,ezp1,ezpp1,ez12,ezp12,ezpp12)
        implicit none
        include 'mpif.h'
        type (TWS), intent(in) :: this
        double precision, intent(in) :: z
...
!          endif
          ...
          !//# of parameters for the second fields.
...
!        call flush_(14)
        ...
        end subroutine getaxfldEold_TWS
        ...
        double precision :: tmpex,tmpey,tmpez,tmpbx,tmpby,tmpbz
        double precision:: zz,bgrad,b0,zmid,rr,zstart1,zend1,zlength1,&
          zstart2,zend2,zlength2,zlen,f1p,r2,zlc,ezppp,theta2,dd
        integer :: i,ntmp,numpar1,numpar2
...
!          call flush_(10)
          ...
          zend2 = Fcoef(7+numpar1) 
          zlength2 = Fcoef(8+numpar1) 
          dd = Fcoef(9+numpar1) !shift distance 
...
!        endif
        ...
        !get external field without displacement and rotation errors.
        subroutine  getfldarray_TWS(zin,phi0,pts,extfld,this,nplc)
        implicit none
        include 'mpif.h'
        integer, intent(in) :: nplc
        real*8, intent(in) :: phi0
        double precision, dimension(9,nplc), intent(in) :: pts
        type (TWS), intent(in) :: this
        double precision, dimension(nplc), intent(out) :: extfld
        ...
        ezpp1=ezpp1*escale
        ezppp = ezppp*escale
!          write(12,*)zin,ez1,ezp1,ezpp1
        do i = 1, nplc
            pos(1) = pts(1,i)*Scxl
            pos(2) = pts(3,i)*Scxl
            pos(3) = pts(5,i) + phi0
            tt = pos(3)/(2*pi*Scfreq)
            tmpcos = cos(ww*tt+theta0)
            tmpsin = sin(ww*tt+theta0)
            f1 = -(ezpp1+ez1*xlrep*xlrep)/4
            f1p = -(ezppp+ezp1*xlrep*xlrep)/4
...
!            tmpbz = 0.0
        ...
        end subroutine getfldarray_TWS

        !get external field with displacement and rotation errors.
        subroutine  getflderrarray_TWS(pos,extfld,this,dx,dy,anglex,angley,&
                                    anglez)
        implicit none
        include 'mpif.h'
        double precision, dimension(4), intent(in) :: pos
...
!        endif
        ...
        end subroutine getflderrarray_TWS

        !get external field without displacement and rotation errors.
        subroutine  getfldold_TWS(pos,extfld,this)
        implicit none
        include 'mpif.h'
        double precision, dimension(4), intent(in) :: pos
        type (TWS), intent(in) :: this
...
!        endif
        ...
        end subroutine getfldold_TWS

        !get external field with displacement and rotation errors.
        subroutine  getflderrold_TWS(pos,extfld,this,dx,dy,anglex,angley,&
                                    anglez)
        implicit none
        include 'mpif.h'
        double precision, dimension(4), intent(in) :: pos
...
!        endif
        ...
        end subroutine getflderrold_TWS
      end module TWSclass
