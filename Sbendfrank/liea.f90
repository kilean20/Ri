...
cryne 5/3/2006      if(allocated(pbh6t))then
c     if(idproc.eq.0)write(6,*)'pbh6t is already allocated'
      ...
      enddo
      enddo
cryne 5/3/2006      endif
c     if(idproc.eq.0)write(6,*)'done with final do loop'
c     do i=1,top(imaxi)
c     write(6,123)i,(pbh(i,j),j=1,6)
c 123 format(i5,10(1x,1pe10.3))
c     enddo
c     if(idproc.eq.0)write(6,*)'leaving brkts'
      return
      end
c
************************************************************************
...
c
      ...
      include 'symp.inc'
c
c      write(6,*) 'first factor in drcat'
c      call pcmap(1,1,0,0,f,mf)
c      write(6,*) 'second factor in drcat'
...
cryne 7/23/2002      double precision temp(923)
      dimension f(monoms),l(monoms),temp(monoms)
      dimension m(6,6)
c
c      do 127 my=1,6
c      write(6,*) my, len(my),
